...
fn main() {
    App::new()
        ...
        .add_state::<GameState>()
        .add_systems(Startup, (setup_cameras, demo))
...
fn demo(runtime: ResMut<TokioTasksRuntime>, mut _commands: Commands) {
    runtime.spawn_background_task(check_blocks);
}

async fn check_blocks(mut _ctx: TaskContext) {
    loop {
        tokio::time::sleep(Duration::from_secs(5)).await;
        let rpc = TezosRpc::new("https://ghostnet.tezos.marigold.dev/".to_string());
        let result_block: Result<Block> = rpc.get_block().block_id(&BlockId::Head).send().await;
